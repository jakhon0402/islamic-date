import React from "react";

export default function Signature({ children, ...props }) {
  return (
    <div className="fixed w-[150px] h-[100px] scale-50  top-[90%] left-1/2 translate-x-[-50%]">
      <div className="flex w-full h-full flex-col justify-center items-center">
        <div>
          <p className="text-md text-zinc-400 text-center font-thin">
            {children}
          </p>
        </div>
        <div>
          <svg className="w-[150px] h-[80px] mt-[-10%] flex justify-center items-center fill-zinc-400 ">
            <g>
              <path
                className="st0"
                d="M29.2,48.9c-5.4,0-7.1-2.7-7.5-3.5c-0.1-0.1,0-0.3,0.1-0.4l1.1-0.9c0.1-0.1,0.3-0.1,0.4,0.1
		c0.5,0.8,2.5,3,7.8,2.4c5.2-0.6,6-6.8,6-9.6c0-0.2-0.2-0.3-0.3-0.3l-9,0.9c-0.2,0-0.4-0.2-0.3-0.4l0.5-1.5c0-0.1,0.1-0.2,0.2-0.2
		l10.6-1c0.2,0,0.3,0.1,0.3,0.3l0.1,0.8c0.3,4.2-0.9,12.4-7.9,13.2C30.6,48.9,29.9,48.9,29.2,48.9z"
              />
              <path
                className="st0"
                d="M40.7,49L39.9,49c-0.3,0-0.6-0.3-0.5-0.6c0.5-2.8,2.3-12.7,5-15c1.1-1,2.5-1.1,3.7-0.4
		c1.8,1.1,3.5,4.4,1.4,14.4c-0.1,0.3-0.3,0.6-0.7,0.6l-1.1,0.2c-0.2,0-0.4-0.2-0.4-0.4c2.1-10.2,0.1-12.7-0.3-13
		c-0.5-0.3-1-0.1-1.3,0.2c-1.7,1.5-3.5,8.7-4.3,13.3C41.4,48.7,41.1,48.9,40.7,49z"
              />
              <polygon
                className="st0"
                points="41.7,43.8 42,41.1 49.1,39.8 49,42.6 	"
              />
              <path
                className="st0"
                d="M53.7,47.9l-1.4,0.2c-0.2,0-0.4-0.2-0.4-0.4l2.8-14.7c0-0.2,0.2-0.3,0.3-0.3l1.4-0.1c0.2,0,0.4,0.2,0.4,0.4
		L54,47.6C53.9,47.8,53.8,47.9,53.7,47.9z"
              />
              <path
                className="st0"
                d="M64.9,18.7l-4,28c0,0.3-0.2,0.5-0.5,0.5l-1,0.1c-0.4,0-0.7-0.3-0.6-0.7l4.6-27.9c0-0.2,0.2-0.4,0.4-0.5
		l0.5-0.1C64.6,18,65,18.3,64.9,18.7z"
              />
              <rect
                x="53.8"
                y="39"
                transform="matrix(0.9752 -0.2215 0.2215 0.9752 -7.4301 13.7295)"
                className="st0"
                width="7.5"
                height="2.1"
              />
              <path
                className="st0"
                d="M69.5,47.6c-0.2,0-0.4,0-0.6,0c-1.7-0.2-3.1-1-4-2.2c-1.1-1.4-1.6-3.5-1.3-5.5c0.5-3.1,2.7-6,6.8-5.7l0,0
		c3.5,0.3,5.3,3.4,5.3,6.4c0,1.6-0.9,4.1-2.6,5.6C72.1,47.1,70.8,47.6,69.5,47.6z M69.7,36.2c-2.6,0-3.8,2.1-4.1,3.9
		c-0.2,1.5,0.1,2.9,0.8,3.9c0.6,0.8,1.5,1.3,2.6,1.4c1,0.1,1.8-0.2,2.6-0.8c1.3-1.2,1.9-3.1,1.9-4.1c0-1.9-1.1-4.1-3.4-4.3l0,0
		C70,36.2,69.9,36.2,69.7,36.2z"
              />
              <path
                className="st0"
                d="M72.2,70.7c0,0-0.1,0.4-0.7,0.4c-0.4,0-0.4-0.6-0.4-0.6l7.5-38.2l6.2,8.9l6.2-28c0,0,0.2-0.2,0.6-0.2
		c0.3,0.1,0.4,0.4,0.4,0.4L86.4,47l-6.6-8.4L72.2,70.7z"
              />
              <path
                className="st0"
                d="M100.4,39.7c-0.2,3.1,0.6,3.7,0,5c-0.4,1.1-2,1.9-3.9,2c-0.3,0-0.5,0-0.8,0c-1.8,0-3.4-0.6-4.5-1.6
		s-1.7-2.5-1.6-4.3c0.2-5.1,2.6-8.3,6.7-8.9c3-0.4,5.2,1.4,5.3,3.5l-1.5,0c-0.4,0-0.7-0.2-0.9-0.5c-0.3-0.5-1.1-1.2-2.6-1
		c-4.1,0.6-4.8,4.6-4.8,6.9c0,3.3,3,3.8,4.6,3.7c2.6-0.2,2-3.1,1.8-4.8L100.4,39.7z"
              />
              <path
                className="st0"
                d="M96.5,40l-0.4-1.5c0-0.1,0.1-0.3,0.2-0.3l5.2,0c0.2,0,0.4,0.2,0.3,0.4l-0.2,1.4c0,0.2-0.2,0.3-0.4,0.3l-4.3,0
		C96.7,40.3,96.5,40.2,96.5,40z"
              />
              <path
                className="st0"
                d="M104.1,45.8h-1.4c-0.2,0-0.4-0.2-0.3-0.4l1.4-12.7c0-0.3,0.3-0.6,0.6-0.6l1-0.1c0.4,0,0.8,0.3,0.7,0.7
		l-1.6,12.9C104.4,45.7,104.3,45.8,104.1,45.8z"
              />
              <path
                className="st0"
                d="M109.2,45.8l-1,0c-0.3,0-0.5-0.2-0.5-0.5l1.3-13.2l2.1,0.2l-1.2,12.9C109.8,45.5,109.5,45.8,109.2,45.8z"
              />
              <path
                className="st0"
                d="M109.6,39.5c-0.2,0-0.3,0-0.5,0v-1.1c4,0.1,5.9-0.7,6.6-1.9c0.5-1,0-2.2-0.8-2.7c-2.2-1.4-4.8,0.2-4.9,0.3
		l-1-1.9c1.4-1.2,4.4-2.4,7.1-0.2c1.7,1.4,1.9,3.9,1.2,5.4C116.7,38.2,115.4,40,109.6,39.5z"
              />
              <path
                className="st0"
                d="M130.4,44.4c-10.4-0.2-20.7-4-21-4.2l0.9-1.9c0.1,0,10.8,5.4,21,5.4l0,0C131.1,44.1,130.8,44.4,130.4,44.4z"
              />
              <ellipse className="st0" cx="136.8" cy="44.4" rx="1.5" ry="1.1" />
            </g>
          </svg>
        </div>
      </div>
    </div>
  );
}
